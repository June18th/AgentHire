 ...
 */
public class WeChatClawBotChannel implements Channel {
    ...
    private WeixinSdk.PollingWorker worker;

    public WeChatClawBotChannel(String baseUrl, String cdnBaseUrl, WxClawBotAccount account, Agent agent, ChannelRegistry channelRegistry, Resource agentWorkspace) {
        this.agent = agent;
        ...
        try {
            ...
            this.mediaDir = workspacePath.resolve("channel").resolve("wx").resolve(account.userId()).resolve("media").toString();
        } catch (Exception e) {
            throw new RuntimeException("Failed to resolve workspace path", e);
        }

        if (StringUtils.isNotBlank(account.botToken())) {
            // Initialize WeixinSdk
            String botToken = account.botToken();
            this.weixinSdk = new WeixinSdk.Builder().baseUrl(baseUrl).cdnBaseUrl(cdnBaseUrl)
    ...
    public void start() {
        ...
        this.worker = weixinSdk.startPolling(message -> {
            try {
