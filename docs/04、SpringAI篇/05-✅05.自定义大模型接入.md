...
SpringAI 已经集成了很多主流大模型的交互，封装成starter供我们直接使用；比如前面使用的智普大模型，就是直接利用`spring-ai-starter-model-zhipuai`来进行大模型的交互。


但总有例外，比如星火的免费模型Spark Lite(非免费的也没有😂)，在官方的教程中就没有找到可以直接使用的 starter。

接下来我们看一下，基于SpringAI，如果我们要接入一个自定义的大模型，该怎么处理？
...
选择 Spark Lite 模型，我已经开通了；没有开通的小伙伴，可以看到上图中 `领取无限量` 这个按钮是激活状态，点击之后，认证账号后即可获取了（未认证的，直接跳转到认账账号进行认证，支持个人/企业认证）。


领取之后，在右边的鉴权信息中，将 ApiPassword 复制出来待用。

#### 2. 阅读官方接口文档
并不是所有的大模型都有统一的规范，无法确保它们都能直接适配SpringAI的传入/传出，因此后续的接入需要重点阅读官方的接口文档，对大模型的交互进行适配。


我们这里就以星火模型为例，使用的是Spark Lite模型，文档地址为：[https://www.xfyun.cn/doc/spark/HTTP调用文档.html](https://www.xfyun.cn/doc/spark/HTTP%E8%B0%83%E7%94%A8%E6%96%87%E6%A1%A3.html)
...
#### 1. 依赖配置
由于不能直接使用官方的starter, 因此需要主动依赖SpringAI的一些核心包
...
关于配置的维护，可以参考官方提供的starter的实现（比如智普的starter）
...
说明：本文以实现一个最基础的大模型交互为例，因此更多的参数相关配置，将留待后续的文章进行扩充实现；这里只介绍最核心的参数。
...
对于`ChatModel`的实现，关键在于 `public ChatResponse call(Prompt prompt)` 这个方法的实现，其他都是非必须的。


对于`call(Prompt prompt)`方法，内部需要实现和大模型的交互，以同步的http协议为例，我们需要做三件事：

+ 将 `prompt` 转换为大模型的传入参数
+ 发起`http`请求
+ 将大模型的返回对象封装为 `ChatResponse` 对象

具体实现中，我们使用Spring的`restClient`作为http交互的工具(对于需要流式异步交互的场景，可以考虑WebClient)，唯一麻烦的就是需要对请求返回的内容进行映射解析。


鉴于此，我们直接定义一个`SparkPOJO`来保存讯飞大模型的返回结果；如下面这个json为大模型的真实返回结果
...
定义 `POJOConvert`，将请求返回的对象转换为 `ChatResponse`。
...
#### 4. 大模型使用示例
完成自定义大模型的交互后，接下来我们试试效果；使用方法基本和前面介绍的没有区别
...
### 三、小结
本文给大家讲清楚了，在没有官方starter 的情况，如何实现自定义大模型的接入过程，其关键在于 `ChatModel` 接口的实现：
...
同时也会在介绍这些高级功能时 ，给出自定义的大模型接入的相关能力扩展，辅助加深理解。文中所有涉及到的代码，可以到项目中获取 [https://github.com/liuyueyi/spring-ai-demo](https://github.com/liuyueyi/spring-ai-demo/tree/master/S05-self-model)
