上一篇快速带大家基于SpringAi创建了一个调用大模型的示例工程，接下来我们进入与大模型交互的细节，这里主要介绍如何使用提示词，好让大模型的输出更加符合我们的预期。

## 一、基础知识
### 1. 提示词（Prompt）
提示词：Prompt，也可以理解为预设，是模型在开始对话时，预先设定的一些内容，这些内容会作为模型的输入参数，从而影响模型输出的结果。
...
### 2. 消息（Message）
用户与大模型的对话，通常是由一个或多个消息组成，其中每个消息都需要与一个角色关联。

SpringAI中，我们使用 `Message` 类来表示一个消息，`Message` 类中包含一个 `MessageType` 字段，用于表示消息的类型。
...
`MessageType`定义的消息类型，和大模型中定义的角色会进行映射。

<!-- 这是一张图片，ocr 内容为： -->
![](./static/02-1.webp)<!-- 这是一张图片，ocr 内容为： -->
![](https://github.com/liuyueyi/spring-ai-demo/raw/master/docs/static/02-1.webp)

通常有四种角色：

+ system: 系统角色，通过分配特定行为给大模型来创建对话的上下文或范围，我们预设的提示词通常会和 `system` 角色关联
+ user: 用户角色，用户输入的文本通常会和 user 角色关联
+ assistant: 助手角色，表示模型生成的文本，大模型生成的答案通常会和 `assistant` 角色关联
+ tool: 工具角色，模型调用的函数会返回一些内容，这些内容会和 `tool` 角色关联

### 3. 提示词模板
提示词实际上是一个字符串，若提示词全部由程序预设，会导致提示词的复用性很差，因此，SpringAI提供了一种模板语法，用于生成提示词，模板语法如下：
...
## 二、实例演示
首先我们需要创建一个SpringAI项目，基本流程同 [创建一个SpringAI-Demo工程](01.创建一个SpringAI-Demo工程.md)

### 1. 提示词基本使用
这里我们同样基于智普的免费大模型进行演示，首先创建一个Controller控制器
...
然后再定义一个对话的接口，还是与之前的demo工程的一样，接收用户传入的文本，不同的地方在于我们会在方法实现中，显式创建一个提示词
...
提示词的创建方式比较简单，直接new一个对象：

第一个参数：string类型：默认表示用户输入的消息

第二个参数：ChatOptions类型：表示模型调用的参数，如模型名称、温度 `temperature`、用户名称等



①、`temperature`** **用于控制生成文本的**随机性或创造性**。数值范围一般在 `0.0` 到 `1.0` 之间（有时也可超出该范围），示例中设置为 `0.7d`，表示适度平衡确定性与多样性。

    - 当 `temperature` 接近 `0.0` 时，输出会趋于确定性和保守，通常选择概率最高的词；
    - 当 `temperature` 接近 `1.0` 或更高时，输出更具多样性和创造性，可能会选择低概率但更有趣的词。

②、`user`** **用于标识请求的发起者，通常是**用户的唯一标识符**（如用户名、ID 等）。主要用途包括（示例中设置为 `"二哥的编程星球&Java 进阶之路"`，可能代表当前请求来源的用户身份标识）：
...
如果我们希望预设一个系统提示词，比如给大模型定义一个身份：“你现在是一个专注于给3-5岁儿童聊天的助手”，那么我们可以这样创建一个提示词：
...
<!-- 这是一张图片，ocr 内容为： -->
![](./static/02-2.webp)<!-- 这是一张图片，ocr 内容为： -->
![](https://github.com/liuyueyi/spring-ai-demo/raw/master/docs/static/02-2.webp)
...
### 2. 提示词模板
接下来我们来看一下提示词模板的使用示例，创建一个角色扮演的接口，我们预设的系统提示词模板为
...
<!-- 这是一张图片，ocr 内容为： -->
![](./static/02-3.webp)<!-- 这是一张图片，ocr 内容为： -->
![](https://github.com/liuyueyi/spring-ai-demo/raw/master/docs/static/02-3.webp)

从上面的输出也可以看出，当我传入不同的角色描述，返回的对话信息也会根据我的描述进行显著的变化

### 3. 提示词高级使用
通过`promptTemplate.create`创建的提示词，默认创建的是`UserMessage`类型的消息；如果我们希望创建的是系统提示词呢？可以使用`SystemPromptTemplate`
