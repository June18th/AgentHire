...
本文将演示如何借助 LangGraph4j + SpringAI 来开发一个完整的智能体应用，实现用户传入地址、大模型通过 Function Call 来获取天气，调用大模型的旅游项目推荐能力、美食推荐能力，给用户返回一个旅游攻略。

### 一、项目创建
#### 1. 工程创建
首先我们需要创建一个 LangGraph4j + SpringAI的项目，对这块不熟悉的小伙伴，请参考 [创建一个Langgraph4j示例工程](https://github.com/liuyueyi/spring-ai-demo/blob/master/docs/14.%E5%88%9B%E5%BB%BA%E4%B8%80%E4%B8%AALanggraph4j%E7%A4%BA%E4%BE%8B%E5%B7%A5%E7%A8%8B.md)

#### 2. 核心依赖
本工程主要使用框架为 `SpringAI1.0.1` + `LangGraph4j1.6.0-rc4` + `SpringBoot3.5.4`


对应的大模型为： `zhipu`，通过 SpringMVC 提供访问接口
...
注意：选择大模型时，需要判断是否支持 Function Call，如果不支持，会影响项目的实际运行效果。
...
#### 1. Agent 的核心功能
1. 用户输入提示词
2. 通过 WeatherAgent 获取目标城市的天气情况
3. 如果不需要旅游推荐，直接返回
4. 否则调用 TravelAgent，获取目标城市附近的旅游项目推荐；根据选定的旅游项目，调用 FoodAgent 获取附近的美食推荐
5. 将结果整合，以图文的方式返回给用户
...
#### 2. 实现通用的AgentService
上面几个 Agent 都需要与大模型进行交互，因此可以抽象出一个通用的 AgentService，用于处理 Agent 与大模型的交互。
...
因为大模型本身是无法获取天气信息的，因此这里使用 Function Call 的方式，给大模型装配上天气查询的能力（当然，这里也可以直接使用天气相关的 McpServer）。


这里我们实现一个简单的 WeatherTool 来模拟天气的查询（真实场景中，可以调用第三方的天气服务查询API）。
...
定义一个 `TravelState` 继承自 `AgentState`，来实现多智能体之间的数据共享，由于我们需要 4 个智能体，所以要为每个智能体的返回都定义一个对应的参数进行承接。
...
+ **INPUT**：最开始传入的输入参数；
+ **WEATHER**：由 WeatherAgent 执行后返回的天气信息；
+ **TRAVEL**：由 TravelRecommendAgent 执行后返回的旅游推荐项目；
+ **FOOD**：由 FoodRecommendAgent 执行后返回的美食推荐；
+ **BLOG**：由 XhsBlogGenerateAgent 执行后生成的小红书风格博文内容。


需要注意的是，不同的 Agent 返回的数据类型并不统一：有的可能是基本类型或字符串，有的可能是集合，也可能是自定义的 Java POJO 对象。


**LangGraph4j** 为了支持这些多样化的数据结构，自己实现了一套序列化协议。但在实际开发中，如果返回的是复杂对象（比如 POJO 或集合），我们需要显式提供一个序列化方案，来保证这些对象能够在状态流转中被正确处理。


这里我们选择先实现一个 **JsonSerializer**，作为统一的处理器，把非简单对象序列化为 JSON 字符串进行存储和传递。这么做有几个好处：

+ **兼容性**：JSON 是跨语言通用格式，方便调试与持久化；
+ **统一性**：屏蔽不同对象的差异，让状态传递更顺滑；
+ **可扩展性**：如果后续要换成其他序列化方式（比如 Protobuf、Avro），只需要替换序列化实现即可。
...
#### 4. 天气智能体 WeatherAgent
接下来我们实现第一个智能体，用于获取目标城市的天气。
...
整体比较简单，核心在于 `callWeatherAgent`方法，我们会将它包装为`LangGraph4J`的 Node，这个方法可以理解为 `NodeAction` 的实现，对应使用层 Node 的注册如下。
...
同样的，我们可以在 `TravelState` 中，提供一个直接获取天气的方法，返回的类型为 String。
...
#### 5. 推荐游玩项目智能体TravelRecommendAgent
接下来我们实现第二个智能体，用于推荐目标城市的游玩项目。
...
在游玩项目推荐的实现中，我们借助天气智能体的天气信息，作为提示词，并将大模型的返回结果保存到 State 中供后续的节点使用；同样的，我们在 `TravelState` 中，提供获取游玩项目的方法。
...
#### 6. 美食智能体 FoodAgent
接下来我们实现第三个智能体，基于游玩项目进行美食推荐（如果条件允许，推荐借助百度/高德地图的 MCPServer 来实现）。
...
这个 Agent 的实现更复杂一点，借助天气智能体和游玩项目智能体，通过 SpringAI 的提示词模板 + 结构化输出来实现美食推荐。继续在 `TravelState` 中提供获取美食推荐列表的方法。
...
#### 7. 图文智能体 XhsBlogGenerateAgent
最后，我们实现第四个智能体，基于美食推荐，生成小红书图文。


这里做了一个分支处理，如果是由 WeatherAgent 转过来的，直接作为天气预报 Agent；如果是由 FoodAgent 转过来的，那么作为小红书的旅游美食推荐 Agent。
...
同样的，我们可以在 `TravelState` 中，提供获取最终图文数据的方法
...
#### 8. TravelState序列化
四个智能体实现完毕后，`TravelState`中存储的上下文也都可以确定了，接下来我们定义它的序列化方式。
...
#### 9. 调用多智能体AgentExecutor
接下来需要实现 `AgentExecutor`，在这里定义`StateGraph`，执行多智能体的调用。
...
核心是`GraphBuilder.build()` 方法，定义了 AgentState、Node、Edge、ConditionEdge，通过他们构建了一个有向图，我们可以通过下面这个方法打印下多智能体执行的 PantUML。
...
### 三、小结
本文通过逐步拆解的方式，给大家演示了如何借助 `SpringAI` + `LangGraphJ` + `智普大模型` 从 0 到 1 创建一个多智能体应用，虽然效果不是特别炫酷，但整个业务的流程还是很完整的。


有兴趣的小伙伴也可以实际的体验一下。


虽然看起来实现步骤不多，但涉及到的原理和知识点还是比较多的。
