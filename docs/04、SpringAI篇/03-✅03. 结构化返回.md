通常情况下，如果我们不显式要求大模型返回什么样的数据结构时，大模型返回的数据结构往往是混乱的；这对于上层的业务开发来说，如何将大模型返回的关键信息映射为结构化的数据模型是一个非常困难的事情。


SpringAI提供了一系列转换器来解决上面的痛点；接下来我们来具体看一下，怎么处理返回结果。
...
借助`BeanOutputConverter`可以将返回结果映射为java的POJO类，首先需要我们定义一个提示词模板，比如说查询某个导演的作品
...
为什么上面的方式就可以实现结果映射为java的POJO类？


 我们可以debug一下`ChatClient.create(chatModel).prompt(prompt).call()`返回的对象。
...
从多次体验的结果来看，结果的格式与定义的POJO类一致，因此，基于BeanOutputConverter的实现，可以达到我们想要的结果。


但是在有限的几次访问尝试中，发现`ChatClient`方式返回的结果中actor可能为null，没有正确获取到值，这也侧面说明，大模型返回数据不可控。
...
上面介绍的是返回一个POJO，接下来看一下如何基于`MapOutputConverter`来实现map接收返回结果。


这里使用的是 `ParameterizedTypeReference` 来指定返回结果的类型
...
两个接口的返回结果如下图，虽然都是返回的Map，但是仔细看之后，会发现他们的层级并不一样，基于`ChatClient`返回的层级会多一层，返回的电影被放在了`movie`属性下，以列表的方式组织；而基于`MapOutputConverter`返回的就是一层的map，key为数字；至于孰优孰劣，看个人喜好了。


![](https://github.com/liuyueyi/spring-ai-demo/raw/master/docs/static/03-4.webp)

### 5. ListOutputConverter
除了返回Map的case之外，再看一下返回列表的场景，借助`ListOutputConverter`来实现，基本上和前面介绍的差异不大
...
此外，发挥作用的核心是`Converter`，SpringAI官方提供了下面这些具体的实现，基本上可以覆盖90%以上的业务场景； 若覆盖补全，则考虑通过自定义Converter来实现
