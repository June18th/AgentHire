# 08-✅08.支持鉴权的MCP Server

上一篇 [实现了一个简单的Mcp Server](https://github.com/liuyueyi/spring-ai-demo/blob/master/docs/07.%E5%AE%9E%E7%8E%B0%E4%B8%80%E4%B8%AA%E7%AE%80%E5%8D%95%E7%9A%84McpServer.md)，接下来我们尝试进行补全，比如给其加上权限管控，避免服务被白嫖。

### 一、MCP Server搭建
直接在前面搭建的Mcp Server上进行能力扩展，因此整个项目的搭建和相关配置与上文一致，这里不再赘述过程。
...
### 二、权限管控
我们这里直接采用Http的权限管控，即在请求头中添加`Authorization`字段，值为`Bearer <token>` 或者 `Basic <user:password>`。


为了对 MCP Server 进行权限管理，我们考虑通过自定义的`Filter`来实现，具体逻辑为：
...
+ 注解`@WebFilter` 表示这是一个过滤器，并且支持异步（必须开启，否则mcp客户端无法正常连接）
+ 拦截的url为`/sse`或者`/mcp/messages`: 分别处理sse请求和mcp请求，将他们与其他的请求区分开


好，从请求头中获取`Authorization`字段，判断是否满足要求，然后调整启动类，支持扫描自定义的过滤器。
...
如果想要在 TRAE 中继续使用这个 MCP Server，需要调整MCP的配置，在之前的基础上，加一个请求头。
...
接下来走一个验证对比，分别是没有添加权限的，和加了权限管控的
...
当然，这里只是简单的实现。实际生产中，我们可以通过数据库来实现权限的校验，也可以通过其他方式来实现权限的验证，比如JWT+OAuth2.0。


但不管是哪种方式，核心的原理依然是web鉴权这一套，无非是应用的场景不同，一个是web用户、一个是mcp客户端而已。
