...
The `/applications` daily action cards also link back into action scopes: "今日要投递" opens today-deadline action items, while "今日要跟进" and "已逾期" open overdue follow-up action items. This keeps the dashboard, weekly review, and action list on one cleanup path.

The `/applications` overview risk cards use the same cleanup path: "已投递及后续" opens stale-submitted review, and "待跟进" opens overdue follow-up action items.
