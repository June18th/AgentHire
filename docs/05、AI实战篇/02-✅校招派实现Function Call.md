...
<!-- 这是一张图片，ocr 内容为：MODEL APPLICATION USER PROMPT AND FUNCTION DECLARATION PROMPT FUNCTION IDENTIFIER AND PARAMETERS REQUEST FUNCTION CALL API RESPONSE RESPONSE FROM API RESPONSE RESPONSE -->
...
## 4.开启日志，验证结果
在配置文件 `application-ai.yml` 中，打开ai交互日志，方便观察过程（大模型交互通过<font style="color:#000000;background-color:#c7edcc;">SimpleLoggerAdvisor</font> 实现的日志输出）
...
# 二、小结
这一篇主要介绍了校招派如何通过 Function Call 来给大模型进行赋能。
