...
export default function LlmMonitorPage() {
    ...
    const load = (p: number) => {
        ...
        fetchLlmCalls(true, p, size).then(r => { setCalls(r.list); setTotal(r.total); setPage(r.page) });
    };
    useEffect(() => load(1), []);

    return <main className="p-8 space-y-6"><h1 className="text-2xl font-bold">大模型监控</h1>
        <div className="grid grid-cols-4 gap-4">{[["调用次数", o?.calls ?? 0], ["成功率", `${(o?.successRate ?? 0).toFixed(1)}%`], ["平均耗时", `${(o?.averageDurationMs ?? 0).toFixed(0)} ms`], ["预估费用", `$${o?.estimatedCost ?? 0}`]].map(([k, v]) =>
        ...
        <div className="rounded border bg-white overflow-x-auto">
            <table className="w-full text-sm">
                ...
                <tbody>{calls.map(c => <tr className="border-b cursor-pointer hover:bg-gray-50" key={c.id} onClick={() => fetchLlmCallDetail(true, c.id).then(setDetail)}>
                    <td className="p-3">{new Date(c.createTime).toLocaleString()}</td>
                    <td>{c.nickName || c.jobClawUserId || "-"}</td>
                    <td>{c.channel || "-"}</td>
                    <td>{c.agent || "-"}</td>
                    <td>{c.operation}</td>
                    <td>{c.mode}</td>
        ...
        {detail && <AdminDetailModal detail={detail} onClose={() => setDetail(null)} />}
    </main>;
}

function AdminDetailModal({detail, onClose}: { detail: LlmCallDetail; onClose: () => void }) {
    return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
        <div className="max-h-[80vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3 text-sm">
                <Row label="用户" value={detail.invocation.nickName || detail.invocation.jobClawUserId} />
