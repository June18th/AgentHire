...
package com.git.hui.offer.oc.mcp.server;

import com.fasterxml.jackson.core.type.TypeReference;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.git.hui.offer.oc.mcp.server.session.McpSessionService;
...
public class WebMvcSseServerTransportProvider implements McpServerTransportProvider {
    ...
    @Autowired
    private McpSessionService mcpSessionHelper;
    ...
    private ServerResponse handleMessage(ServerRequest request) {
        ...
        try {
            ...
            if (message instanceof McpSchema.JSONRPCRequest) {
                if (((McpSchema.JSONRPCRequest) message).method().equals("initialize")) {
                    mcpSessionHelper.saveSessionInfo(sessionId, body, null);
                } else {
                    // 判断会话有没有初始化，若没有，则自动初始化
                    if (!mcpSessionHelper.autoInitSession(sessionId, session)) {
                        // 初始化失败，需要McpClient重启
                        return ServerResponse.status(HttpStatus.NOT_FOUND).body(new McpError("Session not found: " + sessionId));
                    }
                }
            } else if (message instanceof McpSchema.JSONRPCNotification) {
                if (((McpSchema.JSONRPCNotification) message).method().equals("notifications/initialized")) {
                    mcpSessionHelper.saveSessionInfo(sessionId, null, body);
                }
            }
