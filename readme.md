...
- [x] 2026/04/23
  - [x] 完成前后端的用户偏好配置改造，支持动态维护，支持用户使用自己的apiKey来取代默认提供的ApiKey
- [x] 2026/04/22
  - [x] 渠道机器人，新增scope，实现不同的权限管控
    - OWNER：表示只有创建这个机器人的用户才可以与其对话
    - LOGIN: 必须绑定了求职派的用户，才可以与他对话 --> 需要到个人中心，绑定用户在对应渠道的 openId，可以通过首次对话时，机器人的返回获取这个需要绑定的id
    - VIP: 必须绑定了求职派，且是VIP的用户
    - PUBLIC: 所有的用户，都可以与它对话
  - [x] 区分群聊、1v1私聊
    - 群聊中，不支持设置用户的个人偏好
    - 当用户绑定了求职派与渠道的openId，那么可以基于求职派用户构建用户偏好信息、保存会话信息；否则用户的偏好、会话信息保存到 openId 下
    - [ ] 飞书的群聊，待验证是否有问题
  - [x] 用户可以配置自己的模型 + ApiKey
    - 用户未配置时，使用通用的模型配置 + ApiKey ---> 即 `agent.ai.preference[0].user-id == total` 的偏好配置
    - 用户配置后，使用自己配置的覆盖默认的模型配置
  - [ ] 前端页面改造
    - [x] 用户个人页，新增渠道配置，支持绑定飞书/钉钉账号，支持添加机器人；支持绑定微信ClawBot
    - [ ] 用户模型相关个人偏好设置
  - [ ] 收敛LLM的调用逻辑、对上层业务封装一个统一的LLM访问层，方便将后续LLM调用的范式封装（如直接访问、ReAct、PlanAndExecute）等
- [x] 2026/04/21
...
- [x] 2026/04/20
  ...
  - [x] 钉钉实现图片、文件、语音内容的本地下载，用于JobFetchAgent的识别
  - [x] 从网络中爬取相关的职位信息
    - [x] 完成要给基于SmartWebFetchTool的网络提取方案
    - [ ] BraveWebSearchTool 的网络搜索方案
    - [ ] Playwright 的网络搜索方案
  - [x] 改为异步解析方案，用户交互之后直接响应；然后再完成之后再告知用户结果
    - [x] 保存抓取结果，待与用户交互之后，确认将结果保存到正式的数据库中
- [x] 2026/04/18
...
- [x] 2026/04/16
  - [x] 引导信息收集过程，目前是同步调用大模型，较慢；需要改造为支持异步流式获取方案
  - [x] 完成飞书通道集成
  - [x] 支持大模型的思考推理过程返回，钉钉/飞书支持显示思考推理过程
  - [ ] 钉钉、飞书支持接受文本、图片等格式内容；支持返回富文本内容

- [x] 2026/04/15
  - 用户相关身份说明文件：`workspace/users/{jobClawUserId}/`目录下
    - agent.md: 操作手册与工作规范，是AI的最高执行准则，通常除了定义我是谁之外，还需要定义如何进行思考
    - soul.md: 用于指定AI助手的灵魂，确保人格设定和工作流程的一致性
    - info.md: 用于指定助手的身份名片，包含名称、角色、专长领域，用于决定AI如何向用户介绍自己
    - user.md: 用户画像，记录用户的个人相关信息，以实现更贴切的推荐服务（会在首次与用户进行对话时，通过多轮对话引导收入信息；并在之后的对话中，定时的更新）
    - tools.md: 工具列表，记录用户使用的工具，用于后续的推荐服务
