...
public class AdminUserController {
    ...
    public Boolean grantRole(Long userId, String roleCode) {
        ...
        Assert.hasText(roleCode, "roleCode must not be blank");
        return rbacService.grantRole(userId, roleCode);
    }

    @RequestMapping(path = "revokeRole")
    public Boolean revokeRole(Long userId, String roleCode) {
        Assert.notNull(userId, "userId must not be null");
        Assert.hasText(roleCode, "roleCode must not be blank");
        return rbacService.revokeRole(userId, roleCode);
    }
}
