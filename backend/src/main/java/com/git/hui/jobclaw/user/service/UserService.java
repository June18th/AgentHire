...
public class UserService implements IUserService {
    ...
    public UserVo detail(Long userId) {
        ...
        UserBo userBo = UserConvert.toBo(user.get());
        vo.setRoleCodes(rbacService.listRoleCodes(userBo));
        vo.setPermissionCodes(rbacService.listPermissionCodes(userBo));
        return vo;
    }
    ...
    public PageListVo<UserVo> searchUserList(UserSearchReq req) {
        ...
        List<UserVo> list = res.getList().stream().map(user -> {
            UserVo vo = UserConvert.toVo(user);
            UserBo userBo = UserConvert.toBo(user);
            vo.setRoleCodes(rbacService.listRoleCodes(userBo));
            vo.setPermissionCodes(rbacService.listPermissionCodes(userBo));
            return vo;
        }).toList();
    ...
    public UserVo updateUserAvatar(Long userId, String avatarUrl) {
        ...
        UserEntity user = userRepository.findById(userId).orElse(null);
        if (user == null) {
            throw new BizException(StatusEnum.RECORDS_NOT_EXISTS, "鐢ㄦ埛涓嶅瓨鍦?");
        }
