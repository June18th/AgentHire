...
import com.git.hui.jobclaw.oc.search.OcSearchIndexService;
...
public class GatherService implements JobInfoPersistService {
    ...
    public GatherVo saveDraftDataList(List<OcDraftEntity> dataList) {
        return saveDraftDataList(dataList, null, null);
    }

    public GatherVo saveDraftDataList(List<OcDraftEntity> dataList, Long sourceId, Long sourceTaskId) {
        ...
        List<OcDraftEntity> unchangedList = new ArrayList<>();
        for (OcDraftEntity data : dataList) {
            data.autoInitVal();
            attachSourceContext(data, sourceId, sourceTaskId);
            ...
            if (existingRecords.isEmpty()) {
                ...
                for (OcDraftEntity existing : existingRecords) {
                    existing.autoInitVal();
                    if (sameDraftContent(existing, data)) {
                        if (attachSourceContext(existing, sourceId, sourceTaskId)) {
                            draftRepository.save(existing);
                        }
                        unchangedList.add(existing);
                        continue;
                    }
                    ...
                    if (sourceId == null) {
                        data.setSourceId(existing.getSourceId());
                    }
                    if (sourceTaskId == null) {
                        data.setSourceTaskId(existing.getSourceTaskId());
                    }
                    data.setUpdateTime(new Date());
                    // 保存更新后的记录
                    draftRepository.save(data);
        ...
        return new GatherVo()
                ...
                .setUnchangedList(unchangedList);
    }

    private boolean attachSourceContext(OcDraftEntity draft, Long sourceId, Long sourceTaskId) {
        boolean changed = false;
        if (sourceId != null && !sourceId.equals(draft.getSourceId())) {
            draft.setSourceId(sourceId);
            changed = true;
        }
        if (sourceTaskId != null && !sourceTaskId.equals(draft.getSourceTaskId())) {
            draft.setSourceTaskId(sourceTaskId);
            changed = true;
        }
        return changed;
    }

    private boolean sameDraftContent(OcDraftEntity existing, OcDraftEntity incoming) {
        return StringUtils.equals(existing.getCompanyName(), incoming.getCompanyName())
                && StringUtils.equals(existing.getCompanyType(), incoming.getCompanyType())
                && StringUtils.equals(existing.getCompanyIndustry(), incoming.getCompanyIndustry())
                && StringUtils.equals(existing.getJobLocation(), incoming.getJobLocation())
                && StringUtils.equals(existing.getRecruitmentType(), incoming.getRecruitmentType())
                && StringUtils.equals(existing.getRecruitmentTarget(), incoming.getRecruitmentTarget())
                && StringUtils.equals(existing.getPosition(), incoming.getPosition())
                && StringUtils.equals(existing.getDeliveryProgress(), incoming.getDeliveryProgress())
                && StringUtils.equals(existing.getLastUpdatedTime(), incoming.getLastUpdatedTime())
                && StringUtils.equals(existing.getDeadline(), incoming.getDeadline())
                && StringUtils.equals(existing.getRelatedLink(), incoming.getRelatedLink())
                && StringUtils.equals(existing.getJobAnnouncement(), incoming.getJobAnnouncement())
                && StringUtils.equals(existing.getInternalReferralCode(), incoming.getInternalReferralCode())
                && StringUtils.equals(existing.getRemarks(), incoming.getRemarks())
                && StringUtils.equals(existing.getSalary(), incoming.getSalary())
                && StringUtils.equals(existing.getEducation(), incoming.getEducation())
                && StringUtils.equals(existing.getExperience(), incoming.getExperience());
    }
...
//        }
    ...
    public List<OcInfoEntity> moveToOc(List<Long> draftIds) {
        ...
        draftData.forEach(draft -> {
            OcInfoEntity ocEntity = ocInfoTransfer.autoFormatDraftInfo(draft);
        ...
        insertList.forEach(searchIndexService::indexJob);
        return insertList;
    }
}
