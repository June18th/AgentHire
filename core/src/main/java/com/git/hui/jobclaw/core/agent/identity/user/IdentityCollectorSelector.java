...
import com.git.hui.jobclaw.core.agent.identity.user.collector.AiBasedIdentityCollector;
import com.git.hui.jobclaw.core.agent.identity.user.collector.RuleBasedIdentityCollector;
...
public class IdentityCollectorSelector {
    private final Map<InfoCollector.CollectorType, InfoCollector> collectorMap;
    ...
    public IdentityCollectorSelector(AiBasedIdentityCollector aiBasedIdentityCollector,
        ...
        this.collectorMap = Map.of(InfoCollector.CollectorType.AI_BASED, aiBasedIdentityCollector,
                InfoCollector.CollectorType.RULE_BASED, ruleBasedIdentityCollector);
    ...
    public InfoCollector getCollector(String jobClawUserId) {
        ...
        if (tmp.isPresent()) {
            ...
            if (collectorType == null) {
                collectorType = InfoCollector.CollectorType.AI_BASED;
            }
            return collectorMap.get(collectorType);
        } else {
            return collectorMap.get(InfoCollector.CollectorType.RULE_BASED);
        }
    }
}
